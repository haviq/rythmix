...
/* native stream URL resolver — reuse download API (proven working path).
   Pre-warms on playSong so URL is ready by the time ExoPlayer needs it. */
const streamCache = new Map(); // videoId -> Promise<url>
function resolveStreamUrl(videoId, { quiet = false } = {}) {
...
function openSongNowPlaying(song) {
  ...
  if (!Player.current) {
    ...
    playSong(song); // click result = play instantly (was previewSong: detail-only)
  } else {
    Player.pending = null;
    renderNowPlaying();
    updateLikeButtons();
...
const NAV = [
  ...
  { id: 'downloads', label: 'Pengelola unduhan', icon: 'i-download', iconActive: 'i-download', hash: '#/downloads' },
  { id: 'settings', label: 'Pengaturan', icon: 'i-more', iconActive: 'i-more', hash: '#/settings' },
...
async function route() {
  ...
  try {
    ...
    else if (parts[0] === 'settings') { setActiveNav('settings'); viewSettings(view); }
    else if (parts[0] === 'downloads') { setActiveNav('downloads'); viewDownloads(view); }
    else if (parts[0] === 'ytm') { setActiveNav('ytm'); viewYTM(view); }
    else {
...
function viewDownloads(view) {
  const hist = store.get('dl_hist', []);
  view.innerHTML = `<div class="page-title">Pengelola unduhan</div>
    ${hist.length
      ? `<div class="track-list">${hist.map((h) => `<div class="lib-row"><img src="${esc(h.thumb || '')}" onerror="this.style.visibility='hidden'"><div class="lr-meta"><div class="lr-t">${esc(h.title)}</div><div class="lr-s">${esc(h.artist || '')}</div></div></div>`).join('')}</div>`
      : emptyHTML('Belum ada unduhan', 'Lagu yang kamu download akan muncul di sini.', { ic: 'i-download' })}`;
}
/* ---- Library ---- */
...
let rmWasPlaying = false;
const __native = typeof RichMusicBridge !== 'undefined';
document.addEventListener('visibilitychange', () => {
  if (__native) return; // ExoPlayer foreground service never auto-pauses — skip web-mode hack
  if (!Player.yt || !Player.ready) return;
  if (document.hidden) {
...
setInterval(() => {
  if (__native) return; // same: no auto-resume watchdog in native mode
