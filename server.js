...
/* stream audio proxy — resolves loader.to job then pipes MP3 bytes directly to client
   ponytail: no caching; each request re-converts. add Redis URL cache when needed */
app.get('/api/stream', async (req, res) => {
  const videoId = String(req.query.v || '');
  if (!/^[\w-]{6,20}$/.test(videoId)) return res.status(400).json({ error: 'bad id' });
  try {
    // 1. start loader.to conversion job
    const startUrl = `${LOADER_API}?format=mp3&url=${encodeURIComponent('https://www.youtube.com/watch?v=' + videoId)}`;
    const startData = await (await fetch(startUrl, { headers: { 'User-Agent': DL_UA, Referer: 'https://loader.to/' } })).json();
    if (!startData.success || !startData.id) throw new Error('loader refused');

    // 2. poll until download_url ready (max 36s)
    let downloadUrl = null;
    for (let i = 0; i < 12; i++) {
      await new Promise(r => setTimeout(r, 3000));
      const d = await (await fetch(startData.progress_url, { headers: { 'User-Agent': DL_UA } })).json();
      if (d.success && d.download_url) { downloadUrl = d.download_url; break; }
    }
    if (!downloadUrl) return res.status(504).json({ error: 'timeout' });

    // 3. validate host before proxying
    const host = new URL(downloadUrl).hostname;
    if (!['loader.to','savenow.to','affadaffa.com'].some(h => host === h || host.endsWith('.' + h)))
      throw new Error('untrusted download host');

    // 4. fetch MP3 immediately and pipe — URL won't expire mid-stream
    const mp3 = await fetch(downloadUrl, { headers: { 'User-Agent': DL_UA } });
    if (!mp3.ok) throw new Error(`mp3 fetch -> ${mp3.status}`);

    res.setHeader('Content-Type', mp3.headers.get('content-type') || 'audio/mpeg');
...
if (require.main === module) {
  app.listen(PORT, '0.0.0.0', () => console.log(`Rythmix Music running on :${PORT}`));
}
module.exports = app;
